...
impl Texture {
    ...
    pub fn from_image(
        ...
        let data_dimensions = img.dimensions();

        let base = TextureBase::new(data_dimensions);

        let instance = Texture::new(device, label, base).unwrap();

        instance.update_data(&queue, &rgba);
